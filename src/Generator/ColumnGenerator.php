...
class ColumnGenerator extends ArrayObject
{
    ...
    public function getMigrationCommand(): string
    {
        ...
        if (isset(self::$casts[$this->DATA_TYPE])) {
            ...
            $columnArgs = [self::quote($this->COLUMN_NAME), $this->CHARACTER_MAXIMUM_LENGTH];
            ...
            $columnArgs = [self::quote($this->COLUMN_NAME), $this->NUMERIC_PRECISION, $this->NUMERIC_SCALE];
            $category = self::COLUMN_TYPE_FLOAT;
        } else if ($this->DATA_TYPE == 'enum' || $this->DATA_TYPE == 'set') {
            $columnFunc = $this->DATA_TYPE;
            $columnArgs = [self::quote($this->COLUMN_NAME), '['.substr($this->COLUMN_TYPE, (strlen($this->DATA_TYPE) + 1), -1).']'];
        ...
        if ($this->COLUMN_DEFAULT != null) {
            $content .= '->default('.(!in_array($category, [self::COLUMN_TYPE_INTEGER, self::COLUMN_TYPE_FLOAT]) ? self::quote($this->COLUMN_DEFAULT) : $this->COLUMN_DEFAULT).')';
        }
        // ->useCurrent()
